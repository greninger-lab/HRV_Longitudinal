...
Epitopes_DF = pd.read_csv("/Users/administrator/Desktop/HRV_SupFiles/Deep_Mutation/RV_A/epitopes-list_updated.csv")

SurfaceExposure_DF = pd.read_csv("/Users/administrator/Desktop/HRV_SupFiles/Deep_Mutation/SurfaceExposure.csv")
...
Epitopes_DF.to_csv("Epitope_Position_Updated.csv", index=False)
...
RV_Combo2 = pd.concat([RV_A, RV_B, RV_C], axis=0, ignore_index=True)
...
RV_Combo_No = RV_Combo[RV_Combo['iSNV'] != 'Yes']
RV_Combo = RV_Combo[RV_Combo['iSNV'] != 'No']
...
SurfaceExposure_DF_Subset = SurfaceExposure_DF.loc[:, ['position', 'struct_pos']]

SurfaceExposure_DF_Subset = SurfaceExposure_DF_Subset.drop_duplicates()

SurfaceExposure_DF_Subset['position'] = SurfaceExposure_DF_Subset['position'] - 1

RV_Combo2['Surface_Exposure'] = ""

SurfaceExposure_DF_Subset.to_csv('AVG_MAX_RP_B.csv', index=False)

SurfaceExposure_DF_Subset = SurfaceExposure_DF_Subset.rename(columns={'position': '4GB3_Position'})

RV_Combo2 = pd.concat([RV_Combo2, SurfaceExposure_DF_Subset], axis=0, ignore_index=True)
RV_Combo2 = pd.merge(RV_Combo2, SurfaceExposure_DF_Subset, on='4GB3_Position', how='left')

RV_Combo2.to_csv('Surface_Exposure_Annotated.csv', index=False)